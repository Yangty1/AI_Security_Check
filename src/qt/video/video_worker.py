...
class FileProcessThread(QThread):
    ...
    def set_start_config(self, video_path, ai_task, screen_size, model_name="best", tracker_name="deepsort", confidence_threshold=0.35, iou_threshold=0.45, frame_interval=0):
        print('set_start_config')
    ...
    def _init_yolo(self):
        ...
        if self.ai_task == "object_detection":
            ...
            # self.detector = YOLO(os.path.join(ROOT, f"weights\{self.model_name}.pt"))
            print(os.path.join(ROOT, f"weights\{self.model_name}"))
            self.detector.init(
                model_path=os.path.join(ROOT, f"weights\{self.model_name}"),
